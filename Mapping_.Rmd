...
allison_rain=force(rain)%>% filter(storm_id=='Allison-2001')%>% group_by(fips) %>%
  mutate(fips=as.numeric(fips)) %>% summarise(storm_id=storm_id[1],precip=sum(precip))
allison_rain=right_join(ob,allison_rain,'fips')
Allison_hurr <- hurr_tracks %>%  filter(storm_id == "Allison-2001")
```
...
allison_rain$'Rainfall > 175mm' <- cut(allison_rain$precip,c(-1,175,500),c("Unexposed","Exposed"),ordered_result = T)
```
...
geom_sf(data=allison_rain,mapping=aes(fill=`Rainfall > 175mm`))+
theme(plot.title=element_text(hjust=0.3))
```
...
ra.new$precip[ra.new$precip<=175] <- 'Unexposed'
...
#Plotting tmap
t_F = tm_shape(ra.new)+
  tm_polygons(col='precip',title="Rain > 175 (mm)", style="pretty")+
  tm_legend(position=c("right","bottom"))+
  tm_shape(Allison)+
  tm_lines(col='red')+
